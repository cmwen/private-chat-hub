...
void main() {
  ...
  late LmStudioConnectionService service;

  setUp(() async {
    SharedPreferences.setMockInitialValues({});
    prefs = await SharedPreferences.getInstance();
    service = LmStudioConnectionService(prefs);
  });

  group('LmStudioConnectionService', () {
    ...
    test('migrates legacy single connection storage', () async {
      final legacyConnection = LmStudioConnection(
        id: 'default',
        name: 'Legacy',
        host: 'legacy.local',
        apiToken: 'secret',
      ...
      SharedPreferences.setMockInitialValues({
        'lmstudio_connection': jsonEncode(legacyConnection.toJson()),
      });
      ...
      expect(prefs.getString('lmstudio_connection'), isNull);
    });
  });
}
